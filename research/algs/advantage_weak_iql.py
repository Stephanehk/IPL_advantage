...
class Advantage_Weak_IQL(OffPolicyAlgorithm):
    """
    ...
    def _collect_feedback(self, step, total_steps) -> Dict:
        ...
        if self._total_feedback == 0 or self._total_feedback < self.num_uniform_feedback:
            ...
            with torch.no_grad():
                ...
                # Compute reward
                adv = self.network.critic(obs, action)
                # reward = qs - self.dataset.replay_buffer.discount * self._compute_next_V(next_obs)
                adv1, adv2 = torch.chunk(adv, 2, dim=1)  # Shape (E, B_fb * S_fb)
                E, B_times_S = adv1.shape
                assert B_times_S == B_fb * S_fb

                adv1 = adv1.view(E, B_fb, S_fb)
                adv2 = adv2.view(E, B_fb, S_fb)
                logits = adv2.sum(dim=2) - adv1.sum(dim=2)  # Sum across sequence dim
    ...
    def _compute_actor_loss(self, obs, action):
        ...
        with torch.no_grad():
            adv = self.network.critic(obs,action).mean(dim=0)
    ...
    def train_step(self, batch: Dict, step: int, total_steps: int) -> Dict:
        ...
        # print ("at train step")
        #
        # print ((self._last_feedback_step is None or (step - self._last_feedback_step) % self.reward_freq == 0))
        # print (self._total_feedback < self.max_feedback)
        # print (self._total_feedback)
        # print (self.max_feedback)
        # print (step >= self.random_steps)
        # print ("\n")
        if (
        ...
        assert adv.shape[1] == B_total
        E = adv.shape[0]

        # Now re-chunk everything to get the logits
        adv1, adv2, aa = torch.split(adv, split, dim=1)  # Now slips over dim 1 because of ensemble.
        adv1, adv2 = adv1.view(E, B_fb, S_fb), adv2.view(E, B_fb, S_fb)
        logits = adv2.sum(dim=2) - adv1.sum(dim=2)  # Sum across sequence dim, (E, B_fb)
