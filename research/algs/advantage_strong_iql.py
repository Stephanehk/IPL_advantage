...
class Advantage_Strong_IQL(OffPolicyAlgorithm):
    """
    ...
    def _collect_feedback(self, step, total_steps) -> Dict:
        print ("NOT IMPLEMENTED YET FOR REGRET BASED PREFERENCES")
        assert False
        all_metrics = {}
        ...
        if self._total_feedback == 0 or self._total_feedback < self.num_uniform_feedback:
            ...
            with torch.no_grad():
                ...
                adv = qs - self.dataset.replay_buffer.discount * self._compute_next_V(obs)
    ...
    def train_step(self, batch: Dict, step: int, total_steps: int) -> Dict:
        ...
        with torch.no_grad():
            vs = self.network.value(obs)
            v = torch.mean(vs, dim=0, keepdim=True)  # Min trick is not used on value.
            if self.target_clipping:
                q_lim = 1.0 / (self.chi2_coeff * (1 - discount))
                v = torch.clamp(v, min=-q_lim, max=q_lim)
